...
function appendFilesToFileList(
...
export default function AddService() {
  ...
  const [fileList, setFileList] = useState<File[]>([]); // Track files manually
  ...
  function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const combinedFiles = [...fileList, ...newFilesArray];

    setFileList(combinedFiles);

    // Update preview
    const newPreviews = getImageData(event);
    setPreview((prevPreview) => [...prevPreview, ...newPreviews]);

    // Set the media field value
    form.setValue("media", combinedFiles as any);
  }
      ...
      <div className="flex flex-grow flex-col gap-2 container my-8">
        ...
        <Form {...form}>
          ...
          >
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      ref={fileRef.ref} // Use ref from register
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            {/*            // Combine the current files with the newly selected files*/}
            {/*            const combinedFiles = appendFilesToFileList(*/}
            {/*              currentFiles,*/}
            {/*              newFiles,*/}
            {/*            );*/}
            ...
            {/*            // Update the preview*/}
            {/*            const urls = getImageData(event);*/}
            {/*            setPreview((prevPreview) => [...prevPreview, ...urls]);*/}
