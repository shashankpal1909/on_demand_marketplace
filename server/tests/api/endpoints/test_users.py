...
def test_root(test_client):
    ...
    assert response.json() == {"message": "Welcome to the On-Demand Service Marketplace API"}


def test_user_register_as_customer(test_client):
    response = test_client.post(
        "/api/v1/users/register",
        json=UserCreate(username="johndoe", email="johndoe@example.com", name="johndoe", password="password",
                        role="customer").__dict__
